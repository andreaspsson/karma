...
describe('BrowserCollection', () => {
  ...
  describe('calculateExitCode', () => {
    ...
    describe('no tests, with error', () => {
      ...
      it('shall fail if failOnEmptyTestSuite is false', () => {
      ...
      it('shall fail if failOnFailingTestSuite is set', () => {
        const res = collection.calculateExitCode(results, false, undefined, true)
        expect(res).to.be.equal(EXIT_CODE_ERROR)
      })
    })
    describe('no tests, no errors', () => {
      ...
      it('shall pass if failOnFailingTestSuite is true', () => {
        const res = collection.calculateExitCode(results, false, false, true)
        expect(res).to.be.equal(EXIT_CODE_SUCCESS)
      })
    })
